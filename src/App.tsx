...
const App = observer(({ store }: { store: RootStore }) => {
  ...
  const onNewTodoKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.code === 'Enter') {
      store.todosStore.addTodo(todoTitle)
      setTodoTitle('')
      rootStore.globalViewStore.setCurrentSearch('')
    }
  }
  const onNewTodoInputChange = ({
    target: { value },
  }: {
    target: { value: string }
  }) => {
    setTodoTitle(value)
    rootStore.globalViewStore.setCurrentSearch(value)
  }
        ...
        <section className="todoapp">
          ...
          <footer className="footer">
            ...
            <ul className="filters">
              {Object.keys(TodoFilterOptions).map((filter) => {
                const todoFilterOption =
                  TodoFilterOptions[filter as keyof typeof TodoFilterOptions]
                return (
                  <li key={filter}>
                    <a
                      className={`${
                        store.globalViewStore.currentFilter === todoFilterOption
                          ? 'selected'
                          : ''
                      }`}
                      onClick={() => {
            ...
            <button
              ...
              onClick={() => store.todosStore.clearCompleted()}
            >
              Clear completed
            </button>
          </footer>
